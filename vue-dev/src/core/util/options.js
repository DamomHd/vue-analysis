 ...
 */
/**
 * @description: 把 parent 和 child 这两个对象根据一些合并策略，合并成一个新对象并返回
 * @param {*}
 * @return {*}
 */
export function mergeOptions (
  ...
  const options = {}
  let key
  // 将parent每一项合并到options上
  for (key in parent) {
    mergeField(key)
  }
  // 遍历child 将存在于child不在parent上的属性继续调用mergeField函数合并到options
  for (key in child) {
    if (!hasOwn(parent, key)) {
      mergeField(key)
    }
  }
  // 对不同的 key 有着不同的合并策略（策略模式）
...
export function resolveAsset (
  options: Object, //当前实例options属性
  type: string, // 类型 filters directives components...
  id: string,
  warnMissing?: boolean
): any {
  /* istanbul ignore if */
